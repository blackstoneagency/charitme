...
- [~] CHAR-0017 - **Campaign Builder hardening in progress**
  ...
  - Completion Evidence: server-authoritative active/draft state transitions with a shared title/story/goal publication gate and Supabase status-history writes, initial creation history records, non-fatal optional evidence persistence with visible launch warnings, private draft enforcement, resumable draft editing without active-content blocking, session-scoped wizard recovery with cleanup after successful save/publish, authenticated UUID-named media uploads with exact UUID-scoped delete-path validation and 5 MB alignment, filtered campaign search with safe pagination and totals, truthful Stripe-only payout handoff with schema-cache-safe legacy preference compatibility, non-public campaign noindex/owner gating, private-list API protection, dedicated `profiles.payment_methods` persistence, and Stripe Connect readiness required for publication; credential-free Playwright coverage added for public SEO/AEO surfaces, guest builder sign-in gating, and unauthenticated mutation rejection; SEO/AEO Supabase reads now fail open to preserve public rendering during transient CMS outages; typecheck, focused campaign/schema/payout tests (58), and production build passed on 2026-07-22. Browser execution remains pending against a deployment containing this branch.
  - Commit: in progress
