...
| Performance | 🟢 improving | prod home **63→88** (LCP 4.1→3.1s, TBT 640→100ms) by fixing the 292KB→6.7KB oversized logo served sitewide. **Lighthouse (local prod build): home DESKTOP = 99** (LCP 0.7s, TBT 10ms, CLS 0.029 — world-class). **Mobile throttled = 71 (home)/80 (features)**: network metrics pessimistic vs CDN prod, but **CLS 0.124 is layout-based & real** — root-caused to `AnnouncementBanner` (client component renders `null`, then `useEffect` fetches `/api/announcements` and injects the bar **above `<main>`** post-hydration → whole-page downshift). **Fix requires an architectural decision (investigated, deferred for sign-off):** the bar lives in the root layout (`app/layout.tsx`), which is currently **static** (why home = 99). Options each have a cost — (a) read the dismissal cookie via `cookies()` to SSR exactly the right bar → **forces the whole app dynamic**, regressing sitewide static perf; (b) SSR the bar via a cached (`unstable_cache`, ~60s) fetch **without** per-user dismissal → keeps static, but dismissed users get a one-time reverse shift; (c) leave as-is → conditional mobile CLS only when an active, non-dismissed announcement exists. No client-only zero-CLS fix exists (above-the-fold post-fetch injection). **Recommend (b)** but it needs sign-off since it changes root-layout rendering. Note: **dark is the site's DEFAULT theme** (`themeScript` in layout), so the dark-mode a11y fixes this session protect the majority of users. Remaining: unused CSS/JS (lower value) |
