...
- [~] CHAR-0017 - **Campaign Builder hardening in progress**
  ...
  - Tax reporting slice: migration `20260722000000_tax_reporting.sql` adds one idempotent Supabase tax record per completed/refunded donation, backfills historical donations, records campaign recipient/EIN and explicit nonprofit eligibility, and updates email delivery timestamps; `/api/donor/tax-report` provides private annual/all-year CSV or JSON exports and `/donor` exposes the report control. Focused tax-report tests, typecheck, and touched-file lint pass. **Pending staging/live migration application and a real Stripe receipt/report smoke test.**
