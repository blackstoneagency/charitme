...
- [~] CHAR-0017 - **Campaign Builder hardening in progress**
  ...
  - Current verification: 53 web test files, 797 tests passing after restoring guest-first campaign builder access, adding privacy-aware public donor profile structured data, adding CSP regression coverage, and wiring paid featured placement into launch; production build remains green.
  ...
  - Accessibility follow-up: admin Grants create/edit fields now have explicit label/control associations; the touched component lint-checks cleanly.
  - Accessibility follow-up: admin Reports search now has an explicit accessible label and the export surface exposes dialog semantics; the touched component lint-checks cleanly.
  - Accessibility follow-up: admin Users detail/create surfaces now expose dialog semantics, user rows are native buttons, and all add-user/search controls are explicitly labeled; the touched component lint-checks cleanly.
  - Accessibility follow-up: admin sponsor logo fallback now documents its intentional image `onError` handler; the touched component lint-checks cleanly.
  - Accessibility follow-up: admin AI triage and fraud-scan overlays now expose dialog semantics without non-keyboard backdrop handlers; both touched components lint-check cleanly.
  - Accessibility follow-up: admin Campaigns create/edit forms now have explicit label/control associations, feature/pin toggles expose accessible names, and destructive confirmations expose dialog semantics; the touched component lint-checks cleanly.
  - Accessibility follow-up: admin Donations refund/note/action overlays now expose dialog semantics, recent/refunded donation rows are native buttons, and search/export controls are explicitly labeled; the touched component lint-checks cleanly.
  - Accessibility follow-up: admin Users recent-user rows are native buttons, add-user fields and export controls have explicit label/control associations, and the export surface exposes dialog semantics; the touched component lint-checks cleanly.
  - Accessibility follow-up: admin SEO/AEO route override and answer editors now associate every metadata, schema, and priority field with its control; the touched component lint-checks cleanly.
  - Accessibility follow-up: admin Payouts detail/action drawer exposes dialog semantics, payout rows are native buttons, search is labeled, and action radios have accessible names; the touched component lint-checks cleanly.
  - Tax reporting slice: migration `20260722000000_tax_reporting.sql` adds one idempotent Supabase tax record per completed/refunded donation, backfills historical donations, records campaign recipient/EIN and explicit nonprofit eligibility, and updates email delivery timestamps; `/api/donor/tax-report` provides private annual/all-year CSV or JSON exports and `/donor` exposes the report control. The report query now prefers enriched nonprofit verification/receipt settings and falls back to the legacy schema until migration rollout. Focused tax-report tests (4), full suite (801), typecheck, lint, and production build pass. **Pending staging/live migration application and a real Stripe receipt/report smoke test.**
  - Admin schema delivery hardening: `/admin/setup` now includes an idempotent tax-receipt/nonprofit-eligibility chunk and the 17-table marketing admin-RLS chunk, with a contract test keeping the one-click flow aligned to the migrations. Staging/live application and receipt smoke testing remain required.
  - Seed safety hardening: `scripts/seed-marketing-data.mjs` and `scripts/seed-organizer-marketing.mjs` now require `CHARITME_ALLOW_DEMO_SEED=true` and refuse `NODE_ENV=production`; the guard has Node tests and the SQL seed README documents an explicit staging session flag.
  - Live tax-schema audit (2026-07-23): `tax_receipts` and `nonprofit_profiles` expose the expected reporting fields, but the live `campaigns` table is missing `nonprofit_id`; this confirms the tax migration is not fully applied and keeps tax reporting below Production Ready until staging/live migration verification is completed.
  - Migration delivery hardening: the UTF-8-safe catch-up generator now reproduces `supabase/catch_up.sql` with `20260722000000_tax_reporting.sql`, including the campaign nonprofit link, receipt trigger, and historical backfill; future catch-up generation can no longer silently omit the tax migration.
  - Marketing RLS hardening: migration `20260723010000_marketing_admin_policies.sql` adds explicit admin-only policies to all 17 service-role-backed marketing tables; the marketing contract test and global RLS coverage test pass. Live policy introspection remains staging/live verification-gated.
  - Accessibility follow-up: admin Marketing contact and business-lead drawers now expose dialog semantics without non-keyboard backdrop handlers; the admin System settings review overlay and reusable field/toggle controls now expose accessible semantics. Touched admin components lint-check cleanly.
  - Accessibility follow-up: shared sponsor/campaign image fallbacks, notification rows, integration dialogs, messages, payout/team dialogs, campaign settings controls, donor tags, refund selectors, and account settings now use keyboard-safe/native controls. Full web lint is clean with zero warnings/errors.
  - Priority: P0
  - Dependencies: live Supabase auth, storage, and Stripe Connect configuration
  - Completion Evidence: server-authoritative active/draft state transitions with a shared title/story/goal publication gate and Supabase status-history writes, initial creation history records, UUID idempotency keys with duplicate-request recovery, non-fatal optional evidence persistence with visible launch warnings, private draft enforcement, resumable draft editing without active-content blocking, session-scoped wizard recovery with cleanup after successful save/publish, authenticated UUID-named media uploads with exact UUID-scoped delete-path validation and 5 MB alignment, normalized `campaign_media` persistence from validated upload paths and campaign-scoped upload/delete synchronization, dashboard edit cover uploads scoped to the edited campaign, server-derived public URLs for uploaded media, normalized-media-first public campaign galleries, explicit upload response validation and delete-failure recovery, explicit public/owner/admin RLS policies for normalized media metadata, Storage bucket MIME/size/path-policy alignment, filtered campaign search with safe pagination and totals, truthful Stripe-only payout handoff with schema-cache-safe legacy preference compatibility, non-public campaign noindex/owner gating, private-list API protection, dedicated `profiles.payment_methods` persistence, and Stripe Connect readiness required for publication; static and dynamic public SEO/AEO route contract coverage, explicit noindex protection for authenticated surfaces, plus credential-free Playwright coverage for public surfaces, guest builder sign-in gating, and unauthenticated mutation rejection; SEO/AEO Supabase reads now fail open to preserve public rendering during transient CMS outages; focused campaign/SEO lint clean, typecheck, full web suite (53 files, 797 tests), and production build passed on 2026-07-22. Browser execution remains pending against a deployment containing this branch.
  - Commit: d073602 (featured placement launch flow), eebb6e8 (audit-log accessibility), 9318407 (admin-content accessibility), 10e6e8c (Countries accessibility), 9e1bb62 (Grants accessibility), eed7bc7 (Reports accessibility), 304942e (Users accessibility), f770dad (sponsor image fallback accessibility), 1770553 (AI triage/fraud-scan modal accessibility), 29e576e (Campaigns accessibility), aa38c2c (tax reporting), 9b80137 (Donations accessibility), 50777db (Users accessibility), 380cf99 (SEO/AEO accessibility), 3535f98 (Payouts accessibility), 1db3214 (Marketing/System accessibility), and 75ce38a (shared/dashboard accessibility), pushed to `agent/seo-aeo-marketing-engine`; current full web lint is clean, with tax migration and live payment/report smoke tests still verification-gated.
