...
| Frictionless UX | 🟢 improving | draft autosave/recovery + funnel analytics shipped; builder roadmap continues. **PR #51:** Escape-to-close on user-facing modals + keyboard-operable rows/toggles; **donor portal loading skeleton** (was frozen-screen on nav → instant perceived load, matching `.pc-skeleton-block` convention). More `loading.tsx` coverage ongoing |
