-- Rollback for 20260815000000 — RESTORES THE PREVIOUS FUNCTION BODY.
--
...
-- replayed to the migration immediately before 20260815000000, and
-- `scripts/generate-function-rollback.sh` re-derives and byte-compares it.
--
-- Regenerate:  ./scripts/generate-function-rollback.sh 20260815000000 record_donation increment_peer_fundraiser_after_donation

-- Drop the signatures the target migration leaves behind, so a changed
-- parameter list cannot leave two overloads resolvable.
drop function if exists public.increment_peer_fundraiser_after_donation() cascade;
drop function if exists public.record_donation(p_stripe_event_id text, p_campaign_id uuid, p_donor_id uuid, p_amount_cents bigint, p_tip_cents bigint, p_processing_fee_cents bigint, p_message text, p_anonymous boolean, p_stripe_payment_intent_id text, p_stripe_checkout_session_id text) cascade;
