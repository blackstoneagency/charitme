-- ─────────────────────────────────────────────────────────────────────────────
-- HOW TO USE
--   Paste this whole file into the Supabase SQL editor (or run via psql) on the
--   target project and run it ONCE. It creates only what is missing and is safe
--   to re-run — validated idempotent against Postgres 16 (two consecutive runs,
--   zero errors on the second). It contains NO demo/seed data.
--
--   It brings a database that was built from the drifted schema.sql up to the
--   full migration schema (adds fundraising_events, grants, marketing,
--   business_leads, payment-observability, feature_flags, admin_settings, etc.).
--
--   Note: a few statements use Supabase Storage internals (storage.foldername,
--   storage.buckets.file_size_limit). Those resolve on a real Supabase project;
--   they only 404 on a bare Postgres without the Storage extension.
-- ─────────────────────────────────────────────────────────────────────────────
