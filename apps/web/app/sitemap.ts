...
type SitemapQueryResult = { data: unknown[] | null; error: unknown };

export async function queryRows(
  run: () => PromiseLike<SitemapQueryResult>,
): Promise<unknown[]> {
  try {
...
async function safeCampaignColumns() {
  try {
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const cols = await safeCampaignColumns();
  const [
    campaigns,
    events,
    grants,
  ...
  const impactCampaigns = impactCampaignIds.length
    ? await queryRows(() => applyLiveFilters(
