...
const SCHEMA_CHUNKS: { name: string; sql: string }[] = [
  ...
  ` },
  // ── Migration 20260610000000: campaign reward/perk tiers ──────────────────
  { name: 'Reward tiers (campaign_rewards + donations.reward_id + claim RPC)', sql: `
    ...
    do $rw$ begin
      if not exists (select 1 from pg_policies where tablename='campaign_rewards' and policyname='rewards_public_read') then
        create policy rewards_public_read on campaign_rewards for select using (true); end if;
      if not exists (select 1 from pg_policies where tablename='campaign_rewards' and policyname='rewards_owner_write') then
        create policy rewards_owner_write on campaign_rewards for insert with check
          (is_admin() or exists (select 1 from campaigns where campaigns.id = campaign_id and campaigns.user_id = auth.uid())); end if;
      if not exists (select 1 from pg_policies where tablename='campaign_rewards' and policyname='rewards_owner_update') then
        create policy rewards_owner_update on campaign_rewards for update using
          (is_admin() or exists (select 1 from campaigns where campaigns.id = campaign_id and campaigns.user_id = auth.uid())); end if;
      if not exists (select 1 from pg_policies where tablename='campaign_rewards' and policyname='rewards_owner_delete') then
    ...
    select pg_notify('pgrst', 'reload schema');
  ` },
  // ── Migration 20260610100000: multi-currency ───────────────────────────────
  { name: 'Multi-currency (donations.currency + flag)', sql: `
    alter table public.donations
    ...
    select pg_notify('pgrst', 'reload schema');
  ` },
  // ── Migration 20260610110000: embedded widget to 100% ─────────────────────
  { name: 'Embedded donation widget → 100% rollout', sql: `
  ...
  { name: 'Marketing per-campaign tracking links (marketing_utm_links.campaign_id)', sql: `
    ...
    select pg_notify('pgrst', 'reload schema');
  ` },
  { name: 'Tax reporting (receipts + nonprofit eligibility)', sql: `
    ...
    create table if not exists public.tax_receipts (
      ...
      nonprofit_id uuid references public.nonprofit_profiles(id) on delete cascade,
    ...
    begin
      if new.status not in ('completed', 'refunded') then return new; end if;
      ...
      insert into tax_receipts (
        donation_id, donor_id, nonprofit_id, campaign_id, receipt_number, amount_cents,
        tip_cents, processing_fee_cents, currency, is_tax_deductible, tax_deductible,
        tax_deductible_amount_cents, tax_year, campaign_title, recipient_name, recipient_ein,
        ...
        new.campaign_id, v_receipt_number, new.amount_cents, coalesce(new.tip_cents, 0),
        coalesce(new.processing_fee_cents, 0), coalesce(new.currency, 'usd'), v_tax_deductible,
        v_tax_deductible, case when v_tax_deductible and new.status = 'completed' then new.amount_cents else 0 end,
        extract(year from new.created_at)::integer, v_campaign.title, v_recipient_name, v_recipient_ein,
        v_donor_name, v_donor_email, new.created_at,
        case when new.status = 'refunded' then 'refunded' else 'issued' end,
        case when new.status = 'refunded' then coalesce(new.refunded_at, now()) else null end, null
      ) on conflict (donation_id) do update set
        donor_id = excluded.donor_id, nonprofit_id = excluded.nonprofit_id,
        campaign_id = excluded.campaign_id, amount_cents = excluded.amount_cents,
        tip_cents = excluded.tip_cents, processing_fee_cents = excluded.processing_fee_cents,
        currency = excluded.currency, is_tax_deductible = excluded.is_tax_deductible,
        tax_deductible = excluded.tax_deductible,
        tax_deductible_amount_cents = excluded.tax_deductible_amount_cents,
        tax_year = excluded.tax_year, campaign_title = excluded.campaign_title,
        recipient_name = excluded.recipient_name, recipient_ein = excluded.recipient_ein,
        donor_name = excluded.donor_name, donor_email = excluded.donor_email,
        donation_date = excluded.donation_date, status = excluded.status,
        refunded_at = excluded.refunded_at;
      return new;
    end; $$;

    drop trigger if exists donations_tax_receipt_sync on public.donations;
    create trigger donations_tax_receipt_sync after insert or update of status, refunded_at on public.donations
    ...
    drop policy if exists tax_receipts_private on public.tax_receipts;
    create policy tax_receipts_private on public.tax_receipts for select
      using (auth.uid() = donor_id or public.is_admin());
    grant all on public.tax_receipts to anon, authenticated, service_role;
    select pg_notify('pgrst', 'reload schema');
  ` },
  { name: 'Marketing admin-only RLS policies', sql: `
    do $$
    ...
    begin
      foreach table_name in array array[
        ...
        execute format('create policy %I on public.%I for all using (public.is_admin()) with check (public.is_admin())', policy_name, table_name);
      end loop;
    end $$;
    select pg_notify('pgrst', 'reload schema');
  ` },
