...
// ─────────────────────────────────────────────────────────────────────────────
// Executes a SQL string against the Supabase project via the pg-meta REST API.
// This works because every Supabase project exposes /pg-meta/v1/query which
// accepts raw SQL authenticated with the service-role JWT.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─── The full production schema split into safe chunks ─────────────────────
// Uses CREATE TABLE IF NOT EXISTS — safe to run multiple times.
// No DROP SCHEMA — preserves existing data.
// Seed data omitted (run separately from Supabase SQL Editor if needed).
const SCHEMA_CHUNKS: { name: string; sql: string }[] = [
  ...
  {
    name: 'is_admin function (after profiles)',
    sql: `
  ...
  {
    name: 'campaign supporting tables',
    sql: `
  ...
  {
    name: 'recurring_donations table',
    sql: `
      create table if not exists public.recurring_donations (
        ...
        next_bill_at timestamptz, cancelled_at timestamptz,
        created_at timestamptz not null default now(), updated_at timestamptz not null default now()
      );
    `,
  },
  {
    name: 'messages and payouts tables',
    sql: `
  ...
  {
    name: 'admin and compliance tables',
    sql: `
  ...
  },
  {
    ...
    sql: `
      ...
      create table if not exists public.support_tickets (
        ...
        status text not null default 'open', campaign_id uuid references campaigns(id) on delete set null,
        created_at timestamptz not null default now(), updated_at timestamptz not null default now()
      );
    `,
  },
    ...
    sql: `
      do $$ begin
        ...
        if not exists (select 1 from pg_policies where tablename='donations' and policyname='donations_read') then
          create policy donations_read on donations for select using (auth.uid() = donor_id or is_admin() or exists (select 1 from campaigns where campaigns.id = campaign_id and campaigns.user_id = auth.uid()));
        end if;
        ...
        if not exists (select 1 from pg_policies where tablename='recurring_donations' and policyname='recurring_own_read') then
          create policy recurring_own_read on recurring_donations for select using (auth.uid() = donor_id or is_admin());
        end if;
        ...
        if not exists (select 1 from pg_policies where tablename='transparency_ledger_items' and policyname='ledger_owner_write') then
        ...
        if not exists (select 1 from pg_policies where tablename='campaign_updates' and policyname='updates_owner_write') then
          create policy updates_owner_write on campaign_updates for insert with check (is_admin() or exists (select 1 from campaigns where campaigns.id = campaign_id and campaigns.user_id = auth.uid()));
        end if;
        ...
        if not exists (select 1 from pg_policies where tablename='integration_connections' and policyname='integrations_own_all') then
          create policy integrations_own_all on integration_connections for all using (auth.uid() = owner_id or is_admin()) with check (auth.uid() = owner_id or is_admin());
        end if;
        ...
        if not exists (select 1 from pg_policies where tablename='support_tickets' and policyname='support_tickets_own') then
          create policy support_tickets_own on support_tickets for select using (auth.uid() = submitter_id or is_admin());
        end if;
      end $$;
    `,
  },
  ...
  },
  {
    ...
    sql: `
      insert into public.platform_settings (id, config) values (1, '{
        ...
        "platformFeePercent": 0, "donationFeePercent": 2.9, "stripeLiveMode": true,
        "allowNewRegistrations": true, "maintenanceMode": false
    ...
    sql: `
      ...
      begin
        ...
        if v_uid is not null then
          ...
          on conflict (id) do update
            set roles = (select jsonb_agg(distinct role) from (select jsonb_array_elements_text(profiles.roles) as role union select 'admin') m),
            updated_at = now();
        end if;
      end; $$;
    `,
...
export async function POST(_request: NextRequest) {
  ...
  for (const chunk of SCHEMA_CHUNKS) {
    ...
    // Stop on first hard error (not idempotency warnings)
    if (!result.ok && result.error && !result.error.includes('already exists')) {
      return NextResponse.json({
        ok: false,
        message: `Failed at chunk: ${chunk.name}`,
        results,
      }, { status: 500 });
    }
  }
  ...
  return NextResponse.json({
    ...
    message: failed.length === 0
      ? `Schema applied successfully! ${results.length} chunks executed.`
      : `Completed with ${failed.length} warnings (already-exist errors are normal).`,
    results,
  });
}
