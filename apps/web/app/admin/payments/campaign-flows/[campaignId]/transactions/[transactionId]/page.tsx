    ...
    <CharitMeShell active="Payment Flows" mode="admin">
      ...
      <div style={{ padding: '0 32px 40px', display: 'grid', gap: 18 }}>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            {timeline.map(([label, complete], index) => (
              <div key={label} style={{ display: 'grid', gridTemplateColumns: '30px 1fr', gap: 10, alignItems: 'center' }}>
                <span style={{ width: 24, height: 24, borderRadius: 999, display: 'grid', placeItems: 'center', background: complete ? '#dcfce7' : '#fee2e2', color: complete ? '#15803d' : '#b91c1c', fontWeight: 900 }}>{index + 1}</span>
                <span style={{ color: '#111944', fontWeight: 850 }}>{label}{complete ? '' : ' missing or pending'}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section style={panel}>
          ...
          <Link href={`https://dashboard.stripe.com/payments/${payment.processor_payment_intent_id ?? ''}`} style={{ color: '#6c35ff', fontWeight: 900, textDecoration: 'none' }}>Open processor dashboard</Link>
        </section>
...
const muted = { color: '#64748b', fontSize: 13, fontWeight: 750 } as const;
