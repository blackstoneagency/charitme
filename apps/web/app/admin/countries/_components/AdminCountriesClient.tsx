      ...
      <div style={{ background: '#fff', border: '1px solid #e8ecf4', borderRadius: 16, padding: '20px 24px', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <label htmlFor="country-flag" style={lStyle}>Flag</label>
            <input id="country-flag" value={newForm.flag_emoji} onChange={e => updNew('flag_emoji', e.target.value)} style={{ ...iStyle, width: 64 }} placeholder="🇺🇸" maxLength={4} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: '2 1 160px' }}>
            <label htmlFor="country-name" style={lStyle}>Country Name *</label>
            <input id="country-name" value={newForm.name} onChange={e => updNew('name', e.target.value)} style={iStyle} placeholder="United States" onKeyDown={e => e.key === 'Enter' && void addCountry()} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <label htmlFor="country-iso-code" style={lStyle}>ISO Code</label>
            <input id="country-iso-code" value={newForm.iso_code} onChange={e => updNew('iso_code', e.target.value.toUpperCase())} style={{ ...iStyle, width: 64 }} placeholder="US" maxLength={2} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <label htmlFor="country-currency" style={lStyle}>Currency</label>
            <input id="country-currency" value={newForm.currency_code} onChange={e => updNew('currency_code', e.target.value.toUpperCase())} style={{ ...iStyle, width: 72 }} placeholder="USD" maxLength={3} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <span style={lStyle}>Fundraise?</span>
            <label htmlFor="country-can-fundraise" style={{ display: 'flex', alignItems: 'center', gap: 6, height: 40, cursor: 'pointer', fontSize: 13, fontWeight: 700, color: newForm.can_fundraise ? '#15803d' : '#94a3b8' }}>
              <input id="country-can-fundraise" type="checkbox" checked={newForm.can_fundraise} onChange={e => updNew('can_fundraise', e.target.checked)} style={{ accentColor: '#6c35ff', width: 16, height: 16 }} />
              {newForm.can_fundraise ? 'Yes' : 'No'}
            </label>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <span style={lStyle}>Donate?</span>
            <label htmlFor="country-can-donate" style={{ display: 'flex', alignItems: 'center', gap: 6, height: 40, cursor: 'pointer', fontSize: 13, fontWeight: 700, color: newForm.can_donate ? '#1d4ed8' : '#94a3b8' }}>
              <input id="country-can-donate" type="checkbox" checked={newForm.can_donate} onChange={e => updNew('can_donate', e.target.checked)} style={{ accentColor: '#6c35ff', width: 16, height: 16 }} />
              {newForm.can_donate ? 'Yes' : 'No'}
            </label>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: '3 1 200px' }}>
            <label htmlFor="country-notes" style={lStyle}>Notes (optional)</label>
            <input id="country-notes" value={newForm.notes ?? ''} onChange={e => updNew('notes', e.target.value)} style={iStyle} placeholder="e.g. Payout via Stripe Connect" />
          </div>
          <button
