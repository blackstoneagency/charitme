...
function TrendChart({ points }: { points: WeekPoint[] }) {
  if (points.length < 2) return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 200, color: 'var(--t3, #8c9ab5)', fontSize: 14 }}>Not enough data</div>;
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        {slices.map(s => (
          <div key={s.label} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 700, color: 'var(--t2, #26335c)' }}>
            <i style={{ width: 8, height: 8, borderRadius: '50%', background: s.color, flexShrink: 0, display: 'block' }} />
            <span>{s.label}</span>
            <b style={{ marginLeft: 'auto', color: 'var(--t1, #0f0f30)' }}>{s.value}</b>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="kf-panel-responsive" style={{ marginLeft: 'auto', width: 500, background: 'var(--s1, #fff)', height: '100%', overflowY: 'auto', boxShadow: '-12px 0 56px rgba(20,20,80,.14)', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--b1, #eef0f7)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: 'var(--t3, #66708d)', marginTop: 2 }}>{payout.recipient_email || 'No email'}</div>
            <div style={{ marginTop: 6, display: 'flex', gap: 6 }}>
              <StatusPill status={payout.status} />
              <span style={{ fontSize: 11, color: 'var(--t3, #8c9ab5)', display: 'inline-flex', alignItems: 'center' }}>ID: {payout.id.slice(0, 12)}…</span>
            </div>
          </div>
          <button type="button" onClick={onClose} style={{ width: 32, height: 32, border: '1px solid var(--b1, #e6e9f2)', borderRadius: '50%', background: 'var(--s1, #fff)', fontSize: 18, cursor: 'pointer', display: 'grid', placeItems: 'center', color: 'var(--t3, #8c9ab5)', lineHeight: 1 }}>×</button>
        </div>

        <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--b1, #eef0f7)', background: 'var(--s2, #fbf9ff)' }}>
          <div style={{ fontSize: 32, fontWeight: 950, color: 'var(--t1, #101944)', letterSpacing: '-.03em' }}>{fmtCents(payout.amount_cents)}</div>
          <div style={{ fontSize: 13, color: 'var(--t3, #66708d)', marginTop: 4 }}>Requested {fmtDate(payout.created_at)}</div>
        </div>

        <div style={{ display: 'flex', borderBottom: '1px solid var(--b1, #eef0f7)', padding: '0 24px' }}>
          {['info', 'history', 'action'].map(t => (
            <button key={t} type="button" onClick={() => setActiveTab(t)}
              style={{ height: 44, border: 0, borderBottom: `2px solid ${activeTab === t ? 'var(--violet, #6c35ff)' : 'transparent'}`, background: 'none', fontWeight: activeTab === t ? 950 : 750, fontSize: 13, color: activeTab === t ? 'var(--violet, #551cf2)' : 'var(--t3, #66708d)', marginRight: 20, cursor: 'pointer' }}>
              {capitalize(t)}
            </button>
          ))}
        </div>
        ...
        <div style={{ padding: '20px 24px', flex: 1 }}>
          {activeTab === 'info' && (
            <div style={{ display: 'grid', gap: 2 }}>
              {[
                ...
                <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--b1, #f0f2f8)', fontSize: 13 }}>
                  <span style={{ color: 'var(--t3, #66708d)', fontWeight: 700 }}>{label}</span>
                  <span style={{ color: 'var(--t1, #101944)', fontWeight: 750 }}>{val}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {activeTab === 'history' && (
            <div style={{ display: 'grid', gap: 12 }}>
              {[
                { event: 'Payout requested', date: payout.created_at, color: 'var(--violet, #6c35ff)' },
                ...
                <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 750, color: 'var(--t1, #101944)' }}>{(ev as { event: string }).event}</div>
                    <div style={{ fontSize: 12, color: 'var(--t3, #8c9ab5)', marginTop: 2 }}>{fmtDate((ev as { date: string }).date)}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {activeTab === 'action' && (
            <div style={{ display: 'grid', gap: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 13, color: 'var(--t3, #66708d)', fontWeight: 700 }}>Current Status:</span>
                <StatusPill status={payout.status} />
              </div>
              <div style={{ fontSize: 14, fontWeight: 750, color: 'var(--t1, #101944)' }}>Select Action</div>
              {[
                ...
                <label key={a.value} style={{ display: 'flex', gap: 10, cursor: 'pointer', padding: '12px', border: `1px solid ${actionMode === a.value ? 'var(--violet, #6c35ff)' : 'var(--b1, #e6e9f2)'}`, borderRadius: 10, background: actionMode === a.value ? 'rgba(108,53,255,.08)' : 'var(--s1, #fff)' }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 850, color: 'var(--t1, #101944)' }}>{a.label}</div>
                    <div style={{ fontSize: 12, color: 'var(--t3, #66708d)', marginTop: 2 }}>{a.desc}</div>
                  </div>
                </label>
              ))}
              <label style={{ display: 'grid', gap: 6, fontSize: 13, fontWeight: 750, color: 'var(--t2, #26335c)' }}>
                Add Note
                <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Optional note…" style={{ border: '1px solid var(--b2, #dfe3ee)', borderRadius: 9, padding: '10px 14px', fontSize: 14, minHeight: 80, resize: 'vertical' }} />
              </label>
              <div style={{ display: 'flex', gap: 12 }}>
                <button type="button" onClick={onClose} style={{ flex: 1, height: 42, border: '1px solid var(--b2, #e0e4ef)', borderRadius: 9, background: 'var(--s1, #fff)', fontSize: 13, fontWeight: 750, cursor: 'pointer' }}>Cancel</button>
    ...
    <div style={{ padding: '0 32px 40px' }}>
      ...
      <div className="kf-two-col" style={{ marginBottom: 24 }}>
        <section className="kf-card kf-chart">
          <div className="kf-card-head"><h2>Payouts Over Time</h2><span style={{ fontSize: 12, color: 'var(--t3, #8c9ab5)' }}>Last 8 weeks</span></div>
        ...
        <section className="kf-card">
          ...
          <div style={{ padding: '0 20px 20px', borderTop: '1px solid #f0f2f8', paddingTop: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 950, color: 'var(--t1, #101944)', marginBottom: 12 }}>Top Recipients</div>
            {topRecipients.slice(0, 3).map((r, i) => (
              <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '1px solid var(--b1, #f0f2f8)' }}>
                <span style={{ width: 22, textAlign: 'center', fontSize: 13, fontWeight: 950, color: 'var(--violet, #6c35ff)' }}>#{i + 1}</span>
                <div style={{ flex: 1, fontSize: 13, fontWeight: 750, color: 'var(--t1, #101944)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.name}</div>
                <strong style={{ fontSize: 13, color: 'var(--t1, #101944)' }}>{fmtCents(r.total_cents)}</strong>
              </div>
            ))}
          </div>
        </section>
      ...
      <section className="kf-card" style={{ marginBottom: 24 }}>
        ...
        {payouts.slice(0, 5).map(p => (
          <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 20px', borderBottom: '1px solid var(--b1, #f0f2f8)', cursor: 'pointer' }}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--t3, #66708d)' }}>{p.campaign_title}</div>
            </div>
            <StatusPill status={p.status} />
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 15, fontWeight: 950, color: 'var(--t1, #101944)' }}>{fmtCents(p.amount_cents)}</div>
              <div style={{ fontSize: 11, color: 'var(--t3, #8c9ab5)' }}>{fmtDate(p.created_at)}</div>
            </div>
          </div>
        ))}
      </section>
      ...
      {/* Main table */}
      <section className="kf-card">
        <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid var(--b1, #eef0f7)', padding: '0 20px', overflowX: 'auto' }}>
          {panelTabs.map(t => (
            <button key={t} type="button" onClick={() => handleTabChange(t)}
              style={{ height: 50, padding: '0 14px', border: 0, borderBottom: `2px solid ${activeTab === t ? 'var(--violet, #6c35ff)' : 'transparent'}`, background: 'none', fontWeight: activeTab === t ? 950 : 750, fontSize: 13, color: activeTab === t ? 'var(--violet, #551cf2)' : 'var(--t2, #202b55)', cursor: 'pointer', whiteSpace: 'nowrap' }}>
              {capitalize(t)}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'payouts' && (
          <div>
            <div style={{ display: 'flex', gap: 12, padding: '14px 20px', borderBottom: '1px solid var(--b1, #eef0f7)', flexWrap: 'wrap' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 200, height: 42, border: '1px solid var(--b2, #e0e4ef)', borderRadius: 9, padding: '0 14px', background: 'var(--s1, #fff)' }}>
              ...
              <select value={filterStatus} onChange={e => { setFilterStatus(e.target.value); setPage(0); }} style={{ height: 42, border: '1px solid var(--b2, #e0e4ef)', borderRadius: 9, padding: '0 14px', fontSize: 13, background: 'var(--s1, #fff)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderTop: '1px solid #eef0f7' }}>
              <span style={{ fontSize: 13, color: 'var(--t3, #66708d)' }}>Showing {filtered.length === 0 ? 0 : page * PAGE_SIZE + 1}–{Math.min((page + 1) * PAGE_SIZE, filtered.length)} of {filtered.length}</span>
              <div style={{ display: 'flex', gap: 6 }}>
                <button type="button" disabled={page === 0} onClick={() => setPage(p => p - 1)} style={{ height: 34, padding: '0 14px', border: '1px solid var(--b2, #e0e4ef)', borderRadius: 8, background: 'var(--s1, #fff)', cursor: page === 0 ? 'default' : 'pointer', opacity: page === 0 ? 0.4 : 1, fontSize: 13 }}>← Prev</button>
                <button type="button" disabled={page >= pages - 1} onClick={() => setPage(p => p + 1)} style={{ height: 34, padding: '0 14px', border: '1px solid var(--b2, #e0e4ef)', borderRadius: 8, background: 'var(--s1, #fff)', cursor: page >= pages - 1 ? 'default' : 'pointer', opacity: page >= pages - 1 ? 0.4 : 1, fontSize: 13 }}>Next →</button>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'recurring' && (
          <div style={{ padding: '20px' }}>
            ...
            <p style={{ color: 'var(--t3, #8c9ab5)', fontSize: 14 }}>Recurring payout schedules will appear here.</p>
          </div>
        )}
        ...
        {activeTab === 'add' && (
          <div style={{ padding: '20px' }}>
            ...
            <p style={{ margin: '0 0 20px', fontSize: 13, color: 'var(--t3, #66708d)' }}>Create a payout for an existing platform user. Find User IDs in Admin → Users.</p>
            {addSuccess && (
              <div style={{ padding: '12px 16px', borderRadius: 9, background: 'rgba(22,163,74,.10)', color: 'var(--green-dark, #079447)', fontSize: 13, fontWeight: 850, marginBottom: 16 }}>
                ✓ Payout created successfully. Reload the page to see it in the list.
              </div>
            )}
            <div style={{ maxWidth: 460, display: 'grid', gap: 16 }}>
              <label style={{ display: 'grid', gap: 6, fontSize: 13, fontWeight: 750, color: 'var(--t2, #26335c)' }}>
                Recipient User ID <span style={{ color: 'var(--red, #e11d48)' }}>*</span>
                <input type="text" value={addUserId} onChange={e => setAddUserId(e.target.value)} placeholder="e.g. 12a3bc45-6d7e-..." style={{ height: 42, border: '1px solid var(--b2, #dfe3ee)', borderRadius: 9, padding: '0 14px', fontSize: 14 }} />
              </label>
              <label style={{ display: 'grid', gap: 6, fontSize: 13, fontWeight: 750, color: 'var(--t2, #26335c)' }}>
                Campaign ID (optional)
                <input type="text" value={addCampaignId} onChange={e => setAddCampaignId(e.target.value)} placeholder="Leave blank for unattached payout" style={{ height: 42, border: '1px solid var(--b2, #dfe3ee)', borderRadius: 9, padding: '0 14px', fontSize: 14 }} />
              </label>
              <label style={{ display: 'grid', gap: 6, fontSize: 13, fontWeight: 750, color: 'var(--t2, #26335c)' }}>
                Amount (USD) <span style={{ color: 'var(--red, #e11d48)' }}>*</span>
                <input type="number" min="0.01" step="0.01" value={addAmount} onChange={e => setAddAmount(e.target.value)} placeholder="0.00" style={{ height: 42, border: '1px solid var(--b2, #dfe3ee)', borderRadius: 9, padding: '0 14px', fontSize: 14 }} />
              </label>
              <label style={{ display: 'grid', gap: 6, fontSize: 13, fontWeight: 750, color: 'var(--t2, #26335c)' }}>
                Payout Method
                <select value={addMethod} onChange={e => setAddMethod(e.target.value)} style={{ height: 42, border: '1px solid var(--b2, #dfe3ee)', borderRadius: 9, padding: '0 14px', fontSize: 14 }}>
              ...
              <label style={{ display: 'grid', gap: 6, fontSize: 13, fontWeight: 750, color: 'var(--t2, #26335c)' }}>
                Internal Note
                <textarea value={addNote} onChange={e => setAddNote(e.target.value)} placeholder="Reason for manual payout…" style={{ border: '1px solid var(--b2, #dfe3ee)', borderRadius: 9, padding: '10px 14px', fontSize: 14, minHeight: 80, resize: 'vertical' }} />
              </label>
              {addError && <p style={{ margin: 0, color: 'var(--red, #e11d48)', fontSize: 13, fontWeight: 750 }}>{addError}</p>}
              <div style={{ display: 'flex', gap: 12 }}>
                <button type="button" onClick={() => { setAddUserId(''); setAddCampaignId(''); setAddAmount(''); setAddNote(''); setAddError(''); setAddSuccess(false); }} style={{ flex: 1, height: 42, border: '1px solid var(--b2, #e0e4ef)', borderRadius: 9, background: 'var(--s1, #fff)', fontSize: 13, fontWeight: 750, cursor: 'pointer' }}>Clear</button>
          ...
          <div style={{ padding: '20px' }}>
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              {[
                ...
                <div key={m.name} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', border: '1px solid var(--b1, #e6e9f2)', borderRadius: 12 }}>
                  <div style={{ flex: 1, fontSize: 14, fontWeight: 750, color: 'var(--t1, #101944)' }}>{m.name}</div>
                  {m.isDefault && <span style={{ fontSize: 11, fontWeight: 950, color: 'var(--violet, #551cf2)', background: 'rgba(108,53,255,.10)', padding: '3px 10px', borderRadius: 999 }}>Default</span>}
                  <button type="button" style={{ height: 34, padding: '0 14px', border: '1px solid var(--b2, #e0e4ef)', borderRadius: 8, background: 'var(--s1, #fff)', fontSize: 12, fontWeight: 750, cursor: 'pointer' }}>Configure</button>
                </div>
              ))}
            </div>
            <button type="button" style={{ marginTop: 14, height: 42, padding: '0 20px', border: '1px dashed var(--b2, #d7c9ff)', borderRadius: 9, background: 'var(--s2, #fbf7ff)', color: 'var(--violet, #551cf2)', fontSize: 13, fontWeight: 750, cursor: 'pointer' }}>+ Add Method</button>
          </div>
        )}
        ...
        {activeTab === 'reports' && (
          <div style={{ padding: '20px' }}>
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
              <select style={{ height: 42, border: '1px solid var(--b2, #dfe3ee)', borderRadius: 9, padding: '0 14px', fontSize: 13 }}><option>Payout Summary</option><option>Recipient Report</option><option>Fee Report</option></select>
              <select style={{ height: 42, border: '1px solid var(--b2, #dfe3ee)', borderRadius: 9, padding: '0 14px', fontSize: 13 }}><option>Last 30 Days</option><option>Last 90 Days</option><option>This Year</option></select>
              <select style={{ height: 42, border: '1px solid var(--b2, #dfe3ee)', borderRadius: 9, padding: '0 14px', fontSize: 13 }}><option>By Week</option><option>By Month</option></select>
        ...
        {activeTab === 'export' && (
          <div style={{ padding: '20px' }}>
            ...
            <p style={{ margin: '0 0 20px', fontSize: 13, color: 'var(--t3, #66708d)' }}>{payouts.length.toLocaleString()} total payouts available for export.</p>
            <div style={{ maxWidth: 420, display: 'grid', gap: 16 }}>
              <div style={{ padding: '14px 16px', border: '1px solid var(--b1, #e6e9f2)', borderRadius: 10, background: 'var(--s2, #fbf9ff)', fontSize: 13 }}>
                <div style={{ fontWeight: 850, marginBottom: 4, color: 'var(--t1, #101944)' }}>Columns included:</div>
                <div style={{ color: 'var(--t3, #66708d)' }}>Recipient, Email, Campaign, Amount (USD), Status, Method, Date</div>
              </div>
              <button
                type="button"
              ...
              <button
                type="button"
                style={{ height: 44, border: '1px solid var(--b2, #e0e4ef)', borderRadius: 9, background: 'var(--s1, #fff)', fontSize: 13, fontWeight: 750, cursor: 'pointer', color: 'var(--violet, #551cf2)' }}
          ...
          <div style={{ padding: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              ...
              {!auditLoading && auditEntries === null && (
                <button type="button" onClick={loadAuditLog} style={{ height: 36, padding: '0 16px', border: '1px solid var(--b2, #e0e4ef)', borderRadius: 8, background: 'var(--s1, #fff)', fontSize: 13, fontWeight: 750, cursor: 'pointer' }}>Load Log</button>
              )}
            </div>
            {auditLoading && <div style={{ padding: '24px 0', textAlign: 'center', color: 'var(--t3, #8c9ab5)', fontSize: 14 }}>Loading audit log…</div>}
            {!auditLoading && auditEntries !== null && auditEntries.length === 0 && (
              <div style={{ padding: '24px 0', textAlign: 'center', color: 'var(--t3, #8c9ab5)', fontSize: 14 }}>No payout audit entries yet.</div>
            )}
            {!auditLoading && auditEntries && auditEntries.map(e => {
              ...
              return (
                <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: '1px solid var(--b1, #f0f2f8)', fontSize: 13 }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: color, flexShrink: 0 }} />
                  <div style={{ flex: 1 }}>
                    <span style={{ fontWeight: 750, color: 'var(--t2, #26335c)' }}>{e.action.replace('payout.', '').replace(/_/g, ' ')}</span>
                    <span style={{ color: 'var(--t3, #8c9ab5)', marginLeft: 8, fontSize: 11 }}>by {e.actorName}</span>
                  </div>
                  {amtCents && <strong style={{ color: 'var(--t1, #101944)' }}>{fmtCents(amtCents)}</strong>}
                  <span style={{ color: 'var(--t3, #8c9ab5)', fontSize: 11 }}>{new Date(e.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                </div>
              );
            })}
          </div>
