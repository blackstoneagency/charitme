...
const card: React.CSSProperties = { background: '#fff', border: '1px solid #eef0f7', borderRadius: 14, padding: '20px 24px' };
    ...
    <CharitMeShell active="Marketing" mode="admin">
      <TopBar
        ...
        actions={<div style={{ display: 'flex', gap: 8 }}>
          <Link href="/admin/marketing/campaign-plans" style={{ height: 38, display: 'inline-flex', alignItems: 'center', padding: '0 16px', borderRadius: 10, background: '#f8f9fc', color: '#374151', border: '1px solid #e2e8f0', fontWeight: 700, fontSize: 13, textDecoration: 'none' }}>Campaigns</Link>
      ...
      <div style={{ padding: '0 20px 48px', maxWidth: 1040 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.4fr) minmax(0,1fr)', gap: 16, alignItems: 'start' }}>
          ...
          <div style={{ ...card }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              <div style={{ fontWeight: 800, fontSize: 15, color: '#1e293b' }}>Active goals</div>
              <Link href="/admin/marketing/goals" style={{ fontSize: 12, fontWeight: 700, color: '#6c35ff', textDecoration: 'none' }}>Manage →</Link>
            </div>
          ...
          <div style={{ display: 'grid', gap: 16 }}>
            <div style={{ ...card, background: cc.goalsAwaitingActivation > 0 ? '#fffbeb' : '#fff', borderColor: cc.goalsAwaitingActivation > 0 ? '#fde68a' : '#eef0f7' }}>
              <div style={{ fontWeight: 800, fontSize: 13, color: '#1e293b', marginBottom: 8 }}>Needs your attention</div>
              {cc.goalsAwaitingActivation > 0 ? (
                ...
                <div style={{ fontSize: 13, color: '#94a3b8' }}>Nothing in the queue. No autonomous actions are enabled, so nothing publishes or spends without you.</div>
              )}
            </div>

            <div style={card}>
              <div style={{ fontWeight: 800, fontSize: 13, color: '#1e293b', marginBottom: 10 }}>Marketing pulse</div>
            ...
            <div style={card}>
              <div style={{ fontWeight: 800, fontSize: 13, color: '#1e293b', marginBottom: 10 }}>Data freshness</div>
        ...
        <div style={{ ...card, marginTop: 16 }}>
          <div style={{ fontWeight: 800, fontSize: 15, color: '#1e293b', marginBottom: 4 }}>Recent activity</div>
          <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 12 }}>Every marketing action — human or system — from the audit log.</div>
          {cc.activity.length === 0 ? (
            <div style={{ fontSize: 13, color: '#94a3b8' }}>No marketing actions recorded yet.</div>
          ) : (
...
function DeltaCard({ d }: { d: Delta }) {
  ...
  return (
    <div style={card}>
      <div style={{ fontSize: 12, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '.04em' }}>{d.label}</div>
      <div style={{ fontSize: 26, fontWeight: 800, color: '#1a1a2e', marginTop: 6 }}>{fmtUnit(d.current, d.unit)}</div>
  ...
  return (
    <div style={{ padding: '10px 0', borderBottom: '1px solid #f5f6fa' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
        ...
        <div style={{ fontSize: 11, color: '#94a3b8', whiteSpace: 'nowrap' }}>{METRIC_LABEL[g.target_metric]}</div>
      </div>
      {p.measurable ? (
        <>
          <div style={{ height: 7, background: '#eef0f7', borderRadius: 6, overflow: 'hidden', marginTop: 6 }}>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '1px solid #f5f6fa' }}>
      ...
      <span style={{ fontSize: 13, color: '#334155', fontWeight: 600, flex: 1 }}>
        {a.action.replaceAll('_', ' ')} <span style={{ color: '#94a3b8', fontWeight: 500 }}>· {a.summary}</span>
      </span>
