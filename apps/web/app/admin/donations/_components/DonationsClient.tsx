...
function auditActionColor(action: string): string {
  if (action.includes('refund')) return 'var(--red, #ff3b5f)';
  if (action.includes('receipt')) return '#2563eb';
  if (action.includes('note')) return 'var(--violet, #551cf2)';
  if (action.includes('spam')) return '#f97316';
  return 'var(--green, #0fa456)';
}
...
function TrendLine({ points }: { points: WeekPoint[] }) {
  if (points.length < 2) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 200, color: 'var(--t3, #8c9ab5)', fontSize: 14 }}>
      ...
      <div className="ado-actions-panel">
        ...
        <div style={{ padding: '16px 20px', overflowY: 'auto', flex: 1 }}>
          ...
          <ActionItem
            ...
            color="var(--red, #ff3b5f)" bg="rgba(190,18,60,.06)"
            onClick={onRefund}
            disabled={detail.status === 'refunded'}
          />
          <ActionItem label="Add Note" color="var(--violet, #551cf2)" bg="rgba(108,53,255,.07)" onClick={onNote} />
          <ActionItem
            label={detail.isSpam ? 'Unmark as Spam' : 'Mark as Spam'}
            color="#f97316" bg="rgba(249,115,22,.08)"
            onClick={onSpam}
            loading={spamLoading}
          />
          ...
          {detail.donorId && (
            <ActionItem
              ...
              color="var(--green, #0fa456)" bg="rgba(22,163,74,.08)"
              onClick={() => window.open(`/admin/users?id=${detail.donorId}`, '_blank')}
            />
          )}
          <ActionItem
            label="View Campaign"
            color="var(--violet, #6c35ff)" bg="rgba(108,53,255,.07)"
          ...
          <ActionItem
            ...
            color="var(--t3, #66708d)" bg="var(--s2, #f4f5fa)"
    ...
    <div className="ado-modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="ado-modal">
        ...
        <div className="ado-modal-body">
          <p style={{ margin: '0 0 20px', fontSize: 13, color: 'var(--t3, #66708d)', lineHeight: 1.5 }}>
          ...
          <div className="ado-field">
            ...
            />
            <small style={{ color: 'var(--t3, #8c9ab5)', fontSize: 11 }}>Max: {fmtCents(detail.amountCents)}</small>
          </div>
          <div className="ado-field">
            <label>Reason</label>
    ...
    <div style={{ padding: '0 32px 48px' }}>
      ...
      <div className="ado-donor-row kf-card">
        ...
        <div className="ado-donor-info">
          ...
          {detail.anonymous && <span style={{ color: 'var(--t3, #8c9ab5)', fontSize: 11, fontStyle: 'italic' }}>Anonymous donation</span>}
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 30, fontWeight: 950, color: 'var(--t1, #0f1238)', letterSpacing: '-.02em', lineHeight: 1 }}>
              {fmtCents(detail.amountCents)}
            </div>
            <div style={{ fontSize: 12, color: 'var(--t3, #8c9ab5)', marginTop: 4 }}>{fmtDateTime(detail.createdAt)}</div>
          </div>
      ...
      <div className="ado-detail-grid">
        ...
        <section className="kf-card" style={{ alignSelf: 'flex-start' }}>
          ...
          <div style={{ padding: '4px 20px 20px' }}>
            {timeline.length === 0 ? (
              <p style={{ color: 'var(--t3, #8c9ab5)', fontSize: 13, margin: '16px 0 0' }}>No history available.</p>
            ) : (
                ...
                {timeline.map((ev, i) => (
                  <div key={i} className="ado-timeline-item">
                    ...
                    <div style={{ paddingBottom: i < timeline.length - 1 ? 20 : 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--t1, #101944)' }}>{ev.event}</div>
                      <div style={{ fontSize: 12, color: 'var(--t3, #8c9ab5)', marginTop: 2 }}>{fmtDateTime(ev.date)}</div>
                    </div>
                  </div>
                ))}
              </div>
...
export default function DonationsClient({
  ...
  if (viewState === 'detail') {
    if (detailLoading) {
      return (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 400, color: 'var(--t3, #8c9ab5)', fontSize: 14 }}>
          Loading donation…
        </div>
      );
    }
    ...
    <div style={{ padding: '0 32px 40px' }}>
      {detailLoading && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 500, display: 'grid', placeItems: 'center', background: 'rgba(10,15,60,.25)', backdropFilter: 'blur(2px)' }}>
          <div style={{ background: 'var(--s1, #fff)', borderRadius: 14, padding: '28px 40px', fontSize: 14, color: 'var(--t2, #26335c)', fontWeight: 750 }}>Loading donation…</div>
        </div>
      )}
      ...
      <div className="kf-two-col" style={{ marginBottom: 24 }}>
        <section className="kf-card kf-chart">
          <div className="kf-card-head">
            ...
            <span style={{ fontSize: 12, color: 'var(--t3, #8c9ab5)' }}>Last 8 weeks</span>
          </div>
        ...
        <section className="kf-card">
          ...
          <div style={{ padding: '0 20px 20px' }}>
            ...
            <div style={{ marginTop: 12 }}>
              {top5.map(c => (
                <div key={c.campaign_id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid var(--b1, #f0f2f8)', fontSize: 13 }}>
                  <span style={{ color: 'var(--t2, #26335c)', fontWeight: 700 }}>{c.campaign_title.slice(0, 28)}</span>
                  <div style={{ textAlign: 'right' }}>
                    <strong style={{ color: 'var(--t1, #101944)' }}>{fmtCents(c.total_cents)}</strong>
                    <span style={{ color: 'var(--t3, #8c9ab5)', fontSize: 11, marginLeft: 6 }}>{Math.round((c.total_cents / campaignTotal) * 100)}%</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="kf-two-col" style={{ marginBottom: 24 }}>
        <section className="kf-card">
          ...
          {donations.slice(0, 5).map(d => (
            <div
              key={d.id}
              style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 20px', borderBottom: '1px solid var(--b1, #f0f2f8)', cursor: 'pointer' }}
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 14, fontWeight: 850, color: 'var(--t1, #101944)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.donor_name}</div>
                <div style={{ fontSize: 12, color: 'var(--t3, #66708d)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.campaign_title}</div>
              </div>
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                <div style={{ fontSize: 15, fontWeight: 950, color: 'var(--t1, #101944)' }}>{fmtCents(d.amount_cents)}</div>
                <div style={{ fontSize: 11, color: 'var(--t3, #8c9ab5)' }}>{fmtDate(d.created_at)}</div>
              </div>
            </div>
          ))}
          <div style={{ height: 48, display: 'grid', placeItems: 'center', borderTop: '1px solid #f0f2f8' }}>
            <button type="button" style={{ background: 'none', border: 'none', color: 'var(--violet, #551cf2)', fontWeight: 950, fontSize: 14, cursor: 'pointer' }}
        ...
        <section className="kf-card">
          ...
          {topDonors.slice(0, 5).map((d, i) => (
            <div key={d.donor_id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', borderBottom: '1px solid var(--b1, #f0f2f8)' }}>
              <span style={{ width: 22, textAlign: 'center', fontSize: 12, fontWeight: 900, color: 'var(--t3, #8c9ab5)' }}>{i + 1}</span>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 14, fontWeight: 850, color: 'var(--t1, #101944)' }}>{d.donor_name}</div>
                <div style={{ fontSize: 12, color: 'var(--t3, #66708d)' }}>{d.donation_count} donation{d.donation_count !== 1 ? 's' : ''}</div>
              </div>
              <strong style={{ fontSize: 15, color: 'var(--t1, #101944)' }}>{fmtCents(d.total_cents)}</strong>
            </div>
          ))}
          {topDonors.length === 0 && (
            <div style={{ padding: '24px', textAlign: 'center', color: 'var(--t3, #8c9ab5)', fontSize: 14 }}>No donors yet</div>
          )}
        </section>
      </div>
      ...
      <section className="kf-card" style={{ marginBottom: 24 }}>
        {/* Panel tabs */}
        <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid var(--b1, #eef0f7)', overflowX: 'auto', padding: '0 20px' }}>
          {panelTabs.map(t => (
            <button key={t} type="button" onClick={() => setPanelTab(t)}
              style={{
                height: 50, padding: '0 16px', border: 0,
                borderBottom: `2px solid ${panelTab === t ? 'var(--violet, #6c35ff)' : 'transparent'}`,
                background: 'none', fontWeight: panelTab === t ? 950 : 750, fontSize: 13,
                color: panelTab === t ? 'var(--violet, #551cf2)' : 'var(--t2, #202b55)', cursor: 'pointer', whiteSpace: 'nowrap',
              }}>
              {t}
            </button>
          ))}
        ...
        {panelTab === 'Donations' && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 20px', borderBottom: '1px solid var(--b1, #eef0f7)', flexWrap: 'wrap' }}>
              ...
              <select value={filterStatus} onChange={e => { setFilterStatus(e.target.value); setPage(0); }}
                style={{ height: 42, border: '1px solid var(--b2, #e0e4ef)', borderRadius: 9, padding: '0 14px', fontSize: 13, background: 'var(--s1, #fff)' }}>
              ...
              <button type="button"
                style={{ height: 42, padding: '0 18px', border: '1px solid var(--b2, #e0e4ef)', borderRadius: 9, background: 'var(--s1, #fff)', fontSize: 13, fontWeight: 750, cursor: 'pointer' }}
            ...
            {/* Status sub-tabs */}
            <div style={{ display: 'flex', gap: 0, padding: '0 20px', borderBottom: '1px solid var(--b1, #eef0f7)' }}>
              {statusTabs.map(t => (
                <button key={t.id} type="button"
                  onClick={() => { setFilterStatus(t.id); setPage(0); }}
                  style={{
                    height: 44, padding: '0 14px', border: 0,
                    borderBottom: `2px solid ${filterStatus === t.id ? 'var(--violet, #6c35ff)' : 'transparent'}`,
                    background: 'none', fontWeight: filterStatus === t.id ? 950 : 700, fontSize: 12,
                    color: filterStatus === t.id ? 'var(--violet, #551cf2)' : 'var(--t3, #66708d)', cursor: 'pointer',
                  }}>
                  {t.label}
                </button>
              ))}
            ...
            {/* Pagination */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderTop: '1px solid var(--b1, #eef0f7)' }}>
              <span style={{ fontSize: 13, color: 'var(--t3, #66708d)' }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                <button type="button" disabled={page === 0} onClick={() => setPage(p => p - 1)}
                  style={{ height: 34, padding: '0 14px', border: '1px solid var(--b2, #e0e4ef)', borderRadius: 8, background: 'var(--s1, #fff)', cursor: page === 0 ? 'default' : 'pointer', opacity: page === 0 ? 0.4 : 1, fontSize: 13 }}>
                  ← Prev
                </button>
                <button type="button" disabled={page >= pages - 1} onClick={() => setPage(p => p + 1)}
                  style={{ height: 34, padding: '0 14px', border: '1px solid var(--b2, #e0e4ef)', borderRadius: 8, background: 'var(--s1, #fff)', cursor: page >= pages - 1 ? 'default' : 'pointer', opacity: page >= pages - 1 ? 0.4 : 1, fontSize: 13 }}>
                  Next →
                </button>
              </div>
            </div>
        ...
        {panelTab === 'Donors' && (
          <div style={{ padding: '20px' }}>
            <h3 style={{ margin: '0 0 16px', fontSize: 16, fontWeight: 950, color: 'var(--t1, #0f1238)' }}>All Donors</h3>
            {topDonors.length === 0 && (
          ...
          <div style={{ padding: '20px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 24 }}>
              {[
                ...
                <div key={s.label} style={{ padding: '18px', border: '1px solid var(--b1, #e6e9f2)', borderRadius: 12, background: 'var(--s1, #fff)' }}>
                  <div style={{ fontSize: 12, color: 'var(--t3, #66708d)', marginBottom: 6, fontWeight: 700 }}>{s.label}</div>
            ...
            {(() => {
              ...
              if (recurring.length === 0) {
                return (
                  <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--t3, #8c9ab5)' }}>
                    <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'var(--s2, #f3f0ff)', display: 'grid', placeItems: 'center', margin: '0 auto 14px', color: 'var(--violet, #7c3aed)' }}>
                    ...
                    <strong style={{ display: 'block', color: 'var(--t2, #26335c)', fontSize: 15, fontWeight: 900, marginBottom: 6 }}>No recurring donors found</strong>
        ...
        {panelTab === 'Refunds' && (
          <div style={{ padding: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <h3 style={{ margin: 0, fontSize: 16, fontWeight: 950, color: 'var(--t1, #0f1238)' }}>Refunded Donations</h3>
        ...
        {panelTab === 'Reports' && (
          <div style={{ padding: '20px' }}>
            <h3 style={{ margin: '0 0 20px', fontSize: 16, fontWeight: 950, color: 'var(--t1, #0f1238)' }}>Reports & Analytics</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
              {[
                ...
                <button key={r.label} type="button"
                  style={{ padding: '18px', border: '1px solid var(--b1, #e6e9f2)', borderRadius: 12, background: 'var(--s1, #fff)', textAlign: 'left', cursor: 'pointer' }}
                  ...
                  <div style={{ fontSize: 14, fontWeight: 850, color: 'var(--t1, #0f1238)', marginBottom: 4 }}>{r.label}</div>
                  <div style={{ fontSize: 12, color: 'var(--t3, #8c9ab5)' }}>{r.desc}</div>
                </button>
              ))}
            </div>
          </div>
        ...
        {panelTab === 'Export' && (
          <div style={{ padding: '20px' }}>
            <h3 style={{ margin: '0 0 20px', fontSize: 16, fontWeight: 950, color: 'var(--t1, #0f1238)' }}>Export Data</h3>
...
function AuditTab() {
  ...
  if (loading) return <div style={{ padding: '32px 20px', color: 'var(--t3, #8c9ab5)', fontSize: 14 }}>Loading audit log…</div>;
  if (err) return <div style={{ padding: '20px' }} className="ado-error">{err}</div>;

  return (
    <div style={{ padding: '20px' }}>
      <h3 style={{ margin: '0 0 16px', fontSize: 16, fontWeight: 950, color: 'var(--t1, #0f1238)' }}>Audit Log</h3>
      {entries.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '32px', color: 'var(--t3, #8c9ab5)', fontSize: 14 }}>No audit entries yet.</div>
      ) : (
        entries.map(e => (
          <div key={e.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 14, padding: '12px 0', borderBottom: '1px solid var(--b1, #f0f2f8)' }}>
            <div style={{ width: 10, height: 10, borderRadius: '50%', background: auditActionColor(e.action), marginTop: 4, flexShrink: 0 }} />
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--t2, #26335c)' }}>{auditActionLabel(e.action)}</div>
              <div style={{ fontSize: 11, color: 'var(--t3, #8c9ab5)', marginTop: 2 }}>by {e.actorName}</div>
            </div>
            <span style={{ fontSize: 12, color: 'var(--t3, #8c9ab5)', flexShrink: 0 }}>{fmtDateTime(e.createdAt)}</span>
          </div>
        ))
      )}
    </div>
