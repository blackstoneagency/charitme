...
function LogoImg({ src, website, name, style }: { src: string | null; website: string | null; name: string; style?: React.CSSProperties }) {
  ...
  if (!imgSrc) {
    return <span style={{ fontSize: 11, color: 'var(--t3)', fontWeight: 700 }}>{name.slice(0, 2).toUpperCase()}</span>;
  }
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 20px', background: 'var(--s1)', borderRadius: 12, border: '1px solid #eef0f7', marginBottom: 10, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={() => setEditing(false)}
                style={{ padding: '5px 14px', background: 'var(--s2)', color: 'var(--t3)', borderRadius: 7, border: 'none', cursor: 'pointer', fontSize: 12 }}>Cancel</button>
            </div>
          </div>
        ) : (
          <>
            <div style={{ fontWeight: 650, fontSize: 14, color: 'var(--t1)' }}>{sponsor.name}</div>
            {sponsor.website && <div style={{ fontSize: 12, color: 'var(--t3)', marginTop: 2 }}>{sponsor.website}</div>}
          </>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
        ...
        <button onClick={() => fileRef.current?.click()} disabled={uploading === sponsor.id}
          style={{ padding: '6px 12px', border: '1px solid #6c35ff', borderRadius: 8, background: 'var(--s2)', color: 'var(--brand-text)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
          {uploading === sponsor.id ? 'Uploading…' : sponsor.logo_url ? '↑ Replace' : '↑ Logo'}
        </button>

        {!editing && (
          <button onClick={() => setEditing(true)}
            style={{ padding: '6px 12px', border: '1px solid #e2e8f0', borderRadius: 8, background: 'var(--s2)', color: 'var(--t1)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
            Edit
          </button>
        )}
...
export default function AdminSponsorsClient() {
  ...
  return (
    <div className="kf-admin-dash" style={{ maxWidth: 900 }}>
      ...
      {/* Preview bar */}
      {visible.length > 0 && (
        <div style={{ marginBottom: 28, padding: '18px 24px', background: 'var(--s1)', borderRadius: 14, border: '1px solid #eef0f7' }}>
      ...
      <div style={{ marginBottom: 24, padding: '20px 24px', background: 'var(--s2)', borderRadius: 14, border: '1px solid #eef0f7' }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <input aria-label="New sponsor website URL" value={newSite} onChange={e => setNewSite(e.target.value)} placeholder="Website URL (optional)"
            style={{ flex: '2 1 240px', padding: '9px 14px', borderRadius: 9, border: '1px solid #d1d5db', fontSize: 13, color: 'var(--t3)' }} />
      ...
      {!loading && sponsors.length === 0 && (
        <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--t3)', fontSize: 14 }}>No sponsors yet. Add one above.</div>
      )}
      {sponsors.map(s => (
