        ...
        {editing ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <input
              ...
              style={{ padding: '6px 10px', borderRadius: 7, border: '1px solid #d1d5db', fontSize: 13, fontWeight: 700 }}
              placeholder="Company name"
            />
            <input
              value={editSite}
              onChange={e => setEditSite(e.target.value)}
              style={{ padding: '6px 10px', borderRadius: 7, border: '1px solid #d1d5db', fontSize: 12, color: '#6b7280' }}
              placeholder="https://example.com (optional)"
            />
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => { onEdit(sponsor.id, editName, editSite); setEditing(false); }}
                style={{ padding: '5px 14px', background: '#6c35ff', color: '#fff', borderRadius: 7, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>
                Save
              </button>
              <button onClick={() => setEditing(false)}
                style={{ padding: '5px 14px', background: '#f1f5f9', color: '#64748b', borderRadius: 7, border: 'none', cursor: 'pointer', fontSize: 12 }}>
                Cancel
              </button>
            </div>
          </div>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: 12, fontWeight: 700, color: sponsor.active ? '#16a34a' : '#94a3b8' }}>
          <input
            ...
            onChange={e => onToggle(sponsor.id, e.target.checked)}
...
export default function AdminSponsorsPage() {
  ...
  async function uploadLogo(id: string, file: File) {
    ...
    // Save the URL to the sponsor record
    const pr = await fetch(`/api/admin/sponsors/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ logo_url: d.url }),
    });
      ...
      <div style={{ padding: '0 32px 40px', maxWidth: 900 }}>
        ...
        <div style={{ marginBottom: 24, padding: '20px 24px', background: '#f8f9fc', borderRadius: 14, border: '1px solid #eef0f7' }}>
          ...
          <p style={{ margin: '10px 0 0', fontSize: 12, color: '#94a3b8' }}>
            After adding, click <strong>↑ Logo</strong> on the row to upload the company logo. Only sponsors with logos appear on the homepage.
          </p>
        </div>
