      ...
      <form onSubmit={submit} style={{ background: '#fff', border: '1px solid #eef0f7', borderRadius: 16, padding: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}>
          <div style={{ gridColumn: '1 / -1' }}>
            <label htmlFor="grant-title" style={label}>Title *</label>
          ...
          <div>
            <label htmlFor="grant-funder-name" style={label}>Funder name *</label>
            <input id="grant-funder-name" style={input} value={form.funderName} onChange={(e) => set('funderName', e.target.value)} required placeholder="e.g. Robert Wood Johnson Foundation" />
          </div>
          <div>
            <label htmlFor="grant-funder-type" style={label}>Funder type</label>
            <select id="grant-funder-type" style={input} value={form.funderType} onChange={(e) => set('funderType', e.target.value)}>
              {FUNDER_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
          <div>
            <label htmlFor="grant-category" style={label}>Category</label>
            <input id="grant-category" style={input} value={form.category} onChange={(e) => set('category', e.target.value)} placeholder="e.g. health" />
          </div>
          <div>
            <label htmlFor="grant-status" style={label}>Status</label>
            <select id="grant-status" style={input} value={form.status} onChange={(e) => set('status', e.target.value)}>
              {STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          <div>
            <label htmlFor="grant-amount-min" style={label}>Min award (USD)</label>
            <input id="grant-amount-min" style={input} value={form.amountMin} onChange={(e) => set('amountMin', e.target.value)} inputMode="numeric" placeholder="5000" />
          </div>
          <div>
            <label htmlFor="grant-amount-max" style={label}>Max award (USD)</label>
            <input id="grant-amount-max" style={input} value={form.amountMax} onChange={(e) => set('amountMax', e.target.value)} inputMode="numeric" placeholder="50000" />
          </div>
          <div>
            <label htmlFor="grant-deadline" style={label}>Deadline</label>
            <input id="grant-deadline" style={input} type="date" value={form.deadlineAt} onChange={(e) => set('deadlineAt', e.target.value)} />
          </div>
          <div>
            <label htmlFor="grant-country" style={label}>Country</label>
            <input id="grant-country" style={input} value={form.country} onChange={(e) => set('country', e.target.value)} placeholder="US" />
          </div>
          <div style={{ gridColumn: '1 / -1' }}>
            <label htmlFor="grant-application-url" style={label}>Application URL</label>
            <input id="grant-application-url" style={input} value={form.applicationUrl} onChange={(e) => set('applicationUrl', e.target.value)} placeholder="https://funder.org/apply" />
          </div>
          <div style={{ gridColumn: '1 / -1' }}>
            <label htmlFor="grant-focus-areas" style={label}>Focus areas (comma-separated)</label>
            <input id="grant-focus-areas" style={input} value={form.focusAreas} onChange={(e) => set('focusAreas', e.target.value)} placeholder="health, community, youth" />
          </div>
          <div style={{ gridColumn: '1 / -1' }}>
            <label htmlFor="grant-eligible-entities" style={label}>Eligible entity types (comma-separated)</label>
            <input id="grant-eligible-entities" style={input} value={form.eligibleEntityTypes} onChange={(e) => set('eligibleEntityTypes', e.target.value)} placeholder="nonprofit, school, individual" />
          </div>
          <div style={{ gridColumn: '1 / -1' }}>
            <label htmlFor="grant-summary" style={label}>Summary</label>
          ...
          <div style={{ gridColumn: '1 / -1' }}>
            <label htmlFor="grant-description" style={label}>Full description / eligibility details</label>
