    ...
    <div style={{ ...cardStyle, marginBottom: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 14 }}>
        {campaigns.map((c) => {
          ...
          return (
            <Link key={c.id} href={`/campaigns/${c.slug}`} style={{ display: 'block', minHeight: 44, textDecoration: 'none', color: 'inherit' }}>
              <div style={{ border: '1px solid var(--b1, #f1f5f9)', borderRadius: 12, overflow: 'hidden', height: '100%', display: 'flex', flexDirection: 'column' }}>
                <div style={{
                  height: 110, background: `url(${getDisplayCover(c.cover_image_url, c.category, c.slug, 'donor-saved')}) center/cover`,
                }} />
