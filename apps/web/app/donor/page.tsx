...
export default async function DonorPortalPage() {
  ...
  const { data: taxDonationData } = await getTaxDonationSources({ donorId: user.id, year: currentTaxYear });

  const donations  = (donationData  ?? []) as DonationRow[];
  const recurring  = (recurringData ?? []) as RecurringRow[];
  const taxRecords = buildTaxReportRecords(taxDonationData as TaxDonationSource[], []);
