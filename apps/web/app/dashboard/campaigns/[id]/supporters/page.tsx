...
export default function SupportersPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) {
    return (
      <CharitMeShell active="My Campaigns">
        <TopBar title="My Supporters" subtitle="Loading…" />
        <div style={{ padding: 40, color: 'var(--t3)' }}>Loading supporters…</div>
      </CharitMeShell>
    );
  }
  if (error || !data) {
    return (
      <CharitMeShell active="My Campaigns">
        <TopBar title="My Supporters" subtitle="" />
        <div style={{ padding: 40, color: '#ef4444' }}>{error || 'Not found.'} <Link href="/dashboard/campaigns">← Back to campaigns</Link></div>
      </CharitMeShell>
    );
  }
  ...
  return (
    <CharitMeShell active="My Campaigns">
      <TopBar
        title="My Supporters"
        subtitle={`${data.campaign.title} — know your donors, bring them back.`}
      />
      <div style={{ padding: '0 32px 48px', maxWidth: 1080 }}>
        <Link href={`/dashboard/campaigns/${campaignId}`} style={{ fontSize: 13, color: 'var(--t3)', textDecoration: 'none' }}>← Back to campaign</Link>
