...
export default function CampaignAnalyticsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <div style={{ padding: '48px 24px', textAlign: 'center', color: '#94a3b8' }}>Loading analytics…</div>;
  if (error || !analytics) return <div style={{ padding: '48px 24px', textAlign: 'center', color: '#ef4444' }}>{error || 'No data'}</div>;
  ...
  return (
    <div style={{ padding: '32px 24px', maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <Link href={`/dashboard/campaigns/${campaign.id}`} style={{ color: '#94a3b8', fontSize: 13, textDecoration: 'none' }}>
          ← {campaign.title}
        </Link>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
          <h1 style={{ fontSize: 24, fontWeight: 900, color: '#1a1a2e', margin: 0 }}>Analytics <span style={{ fontSize: 13, fontWeight: 600, color: '#94a3b8' }}>last 30 days</span></h1>
