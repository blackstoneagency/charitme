...
export default function NotificationsPage() {
  ...
  return (
    <div style={{ padding: '32px 24px', maxWidth: 720, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 900, color: '#1a1a2e', margin: '8px 0 0' }}>
            ...
            {unreadCount > 0 && (
              <span style={{
                marginLeft: 10, background: '#6c35ff', color: '#fff',
        ...
        {unreadCount > 0 && (
          <button
            onClick={markAllRead}
            style={{
              background: 'none', border: '1.5px solid #6c35ff',
              color: '#6c35ff', fontWeight: 700, fontSize: 13,
      ...
      {/* Filter tabs */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: '1.5px solid #f0f0f8', paddingBottom: 0 }}>
        {(['all', 'unread'] as const).map(tab => (
          <button
            ...
            style={{
              ...
              color: filter === tab ? '#6c35ff' : '#64748b',
              fontSize: 14, padding: '8px 16px',
              borderBottom: filter === tab ? '2px solid #6c35ff' : '2px solid transparent',
          ...
          {displayed.map(n => (
            <div
              ...
              style={{
                ...
                border: '1px solid',
                borderColor: n.read_at ? '#f0f0f8' : '#e0d4ff',
                transition: 'background .15s',
              }}
            >
              {/* Icon */}
              <div style={{
                width: 40, height: 40, borderRadius: 12, background: '#f5f0ff',
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                  <span style={{
                    fontSize: 10, fontWeight: 700, color: '#6c35ff',
                    background: '#ede9ff', padding: '2px 7px', borderRadius: 5, flexShrink: 0,
                    textTransform: 'uppercase', letterSpacing: '0.05em',
                  }}>
                    {KIND_LABEL[n.kind] ?? 'Info'}
                  </span>
                  <span style={{ fontSize: 11, color: '#94a3b8', flexShrink: 0 }}>{timeAgo(n.created_at)}</span>
                </div>
                <div style={{ fontSize: 14, fontWeight: n.read_at ? 600 : 800, color: '#1a1a2e', lineHeight: 1.4 }}>
                  {n.title}
                </div>
                {n.body && (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8, flexShrink: 0 }}>
                {!n.read_at && (
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#6c35ff' }} />
                )}
                <button
                  ...
                  style={{
                    background: 'none', border: 'none', cursor: 'pointer',
                    color: '#cbd5e1', fontSize: 16, padding: 0, lineHeight: 1,
                  }}
                >
                  ×
                </button>
