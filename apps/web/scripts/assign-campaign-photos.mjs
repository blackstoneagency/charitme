 ...
 *   node scripts/assign-campaign-photos.mjs --emit-migration  # write SQL instead
 *   node scripts/assign-campaign-photos.mjs --commit          # write to Supabase
 *   node scripts/assign-campaign-photos.mjs --limit 20        # sample while testing
 *
 * `--emit-migration` needs NO service-role key: it writes a reviewable migration
 * plus its rollback into supabase/, which the owner applies on a normal deploy —
 * the same mechanism that wrote today's covers
 * (20260903000000_first_party_subject_covers.sql).
 */

import { readFileSync, writeFileSync, mkdirSync, readdirSync } from 'node:fs';
import { resolve, join } from 'node:path';
import { isGeneratedCover, planAssignments, planIsDistinct } from './lib/campaign-photo-plan.mjs';
import { harvestCategory, PHOTO_HOST } from './lib/openverse-photos.mjs';

const argv = process.argv.slice(2);
const COMMIT = argv.includes('--commit');
const EMIT_MIGRATION = argv.includes('--emit-migration');
/**
 * Photo source. `rawpixel` (default) is key-free CC0 professional stock via
 * Openverse; `unsplash` needs UNSPLASH_ACCESS_KEY and is kept because the site
 * already approves that host and the licence is equally usable.
 */
const SOURCE = argv.includes('--source') ? argv[argv.indexOf('--source') + 1] : 'rawpixel';
...
/** JSON fetch with a couple of retries — Openverse rate-limits bursts. */
...
/** Pool enough distinct photos for `need` campaigns. */
async function poolFor(category, need) {
  if (SOURCE === 'rawpixel') return harvestCategory(category, need, fetchJson);
  if (!UNSPLASH) return [];
...
async function main() {
  ...
  console.log(`· assigned ${assignments.length} campaigns ${assignments.length} DISTINCT photos from ${SOURCE}`);
  if (SOURCE === 'rawpixel') {
    const offHost = assignments.filter((a) => !a.photo.url.includes(PHOTO_HOST));
    if (offHost.length) {
      console.error(`\n✗ ${offHost.length} photo(s) are not on ${PHOTO_HOST}, which the CSP allows. Refusing to write.`);
      process.exit(1);
    }
  }
  ...
  if (EMIT_MIGRATION) {
    // ⚠️ The stamp must sort AFTER every existing migration, and "now" does not
    // guarantee that — this repo carries migrations stamped ahead of the current
    // date (20260904…), so a wall-clock stamp of 20260810… would be inserted
    // BEFORE migrations that are already applied. Take the later of now and
    // (latest existing + 1 second).
    const migDirForStamp = resolve(process.cwd(), '..', '..', 'supabase', 'migrations');
    const latest = readdirSync(migDirForStamp)
      .map((f) => (f.match(/^(\d{14})_/) ?? [])[1])
      .filter(Boolean)
      .sort()
      .pop() ?? '0';
    const now = new Date().toISOString().replace(/[-:T]/g, '').slice(0, 14).padEnd(14, '0');
    const stamp = now > latest ? now : String(BigInt(latest) + 1n).padStart(14, '0');
    const name = `${stamp}_campaign_real_cover_photos`;
    const esc = (v) => String(v).replaceAll("'", "''");

    // Each UPDATE is guarded so it can only replace GENERATED art. If an
    // organizer uploads a real cover between generating this file and applying
    // it, the guard skips that row instead of overwriting their photo.
    const guard = `(cover_image_url is null
        or cover_image_url = ''
    ...
    const body = assignments.map((a) =>
      `update public.campaigns set cover_image_url = '${esc(a.photo.url)}'\n `
      + ` where id = '${esc(a.campaign.id)}'::uuid and ${guard};`).join('\n');

    const migration = `-- Generated by scripts/assign-campaign-photos.mjs\n`
      + `-- ${assignments.length} campaigns, ${new Set(assignments.map((a) => a.photo.url)).size} DISTINCT photos.\n`
      + `-- Source: ${SOURCE === 'rawpixel'
        ? 'Rawpixel via Openverse (CC0 public-domain dedication — no attribution required)'
        : 'Unsplash (Unsplash License — commercial use, no attribution required)'}.\n`
    ...
    const rollback = `-- Rollback for ${name}.\n`
      + `-- Restores the first-party generated cover for exactly the rows that\n`
      + `-- migration set, matched on the photo URL it wrote.\n\nbegin;\n`
      + assignments.map((a) =>
        `update public.campaigns set cover_image_url = '${esc(a.campaign.cover_image_url ?? '')}'\n `
        + ` where id = '${esc(a.campaign.id)}'::uuid and cover_image_url = '${esc(a.photo.url)}';`).join('\n')
      + `\ncommit;\n`;

    const migDir = resolve(process.cwd(), '..', '..', 'supabase', 'migrations');
    const rbDir = resolve(process.cwd(), '..', '..', 'supabase', 'rollbacks');
    mkdirSync(rbDir, { recursive: true });
    writeFileSync(join(migDir, `${name}.sql`), migration);
    writeFileSync(join(rbDir, `${name.replace(/^(\d+)_/, '$1_rollback_')}.sql`), rollback);
    console.log(`\n✅ wrote supabase/migrations/${name}.sql (${assignments.length} guarded updates) and its rollback`);
    console.log('   Apply it on your normal deploy — no service-role key needed.');
    return;
  }

  if (!COMMIT) {
    console.log('\n— DRY RUN — pass --emit-migration to write SQL, or --commit to write to Supabase. Sample:');
