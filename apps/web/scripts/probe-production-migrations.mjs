...
const NO_PUBLIC_SIGNAL = [
  ...
  ['20260905000000_web_push_subscriptions', 'the table is RLS-scoped to `user_id = auth.uid()` and every row is written by a signed-in device registering itself, so an anonymous request sees the same empty/401 result whether or not the migration is applied. The one surface that could differ — the Settings toggle — is behind auth AND is additionally gated on a VAPID keypair, so on a deployment without one it reads "not enabled" for both states. The send path degrades rather than failing (no table ⇒ the query errors, the error is caught, the donation completes), so an unapplied migration silently disables push instead of breaking anything; only the migration ledger can distinguish the two'],
