...
const NO_PUBLIC_SIGNAL = [
  ['20260904020002_push_subscriptions', 'the table is RLS-scoped to `auth.uid() = user_id` and holds send capabilities, so no anonymous request can observe it at all — a public probe sees the same empty/denied result applied or not. The send path additionally treats 42P01 exactly like "no subscriptions" (push is a courtesy channel and must never fail the donation webhook), so even the feature behaves identically from outside. Only the migration ledger, or an authenticated subscribe round-trip in staging, can distinguish the two states'],
