...
const PROBES = [
  {
    ...
    // ran, since new rows still get a generated card. Half is unreachable before
    // application (it was 0 of 502) and stable long after.
...
const NO_PUBLIC_SIGNAL = [
  ['20260906000000_tombstone_gotrue_readable', 'both halves are invisible to an anonymous request. The tombstone is one auth user and one profile row — profiles has no public SELECT for an arbitrary id, and confirming it by looking the tombstone up as a USER is exactly what must never work, since it is banned, unconfirmed and has no usable password precisely so nothing can reach it. The repair half rewrites NULL token columns in auth.users, which is never publicly readable at all. There IS an authoritative check, it just needs the service-role key rather than an anonymous GET: `node scripts/ensure-tombstone.mjs` reports 200-readable + banned, and returns non-zero when the row is the unreadable kind. Note this migration is ALREADY APPLIED to production, by hand on 2026-08-11, and verified after: 0 unreadable auth rows where there had been 502, and listUsers sweeping all 4 pages clean. It may not appear in supabase_migrations.schema_migrations, so db push will retry it — which is a no-op, since the inserts are ON CONFLICT and the repair is scoped to IS NULL'],
  ['20260906010000_stripe_money_flow_integrity', 'the new refund, application-fee, connected-payout, and allocation records are service-role writes or owner/admin reads. Public pages intentionally expose neither financial object IDs nor reconciliation state, so only migration replay plus authenticated financial RLS and Stripe webhook tests can prove this migration'],
  ['20260905010000_share_events_native_channels', 'the constraint only rejects a WRITE, and the write is an anonymous POST to /api/share-events that answers `{ok:true}` either way — because the route retries a 23514 under a fallback channel rather than failing. So identical HTTP output before and after, by design: the difference is visible only as the stored `channel` on a row an anonymous reader cannot select. A direct read of share_events, or the isolated staging replay, is authoritative'],
  ...
  ['20260904030000_deleted_user_tombstone', 'the migration inserts one auth user and one profile row, both of which are invisible to an anonymous request: profiles has no public SELECT for an arbitrary id, and auth.users is never publicly readable. A probe could only confirm the tombstone by looking it up as a user, which is exactly what must never work — the row is banned, unconfirmed and has no usable password precisely so nothing can reach it. The application checks for the row itself before every deletion and refuses when it is absent (POST /api/account/delete -> TOMBSTONE_MISSING, 503), so an unapplied migration disables the feature rather than corrupting anything'],
  ['20260904040000_default_support_percent_ten', 'the rendered donate card DOES show the suggested support rate, so this looks probeable — but only its NEGATIVE is conclusive. Reading 15% proves the migration is not applied; reading 10% does not prove it is, because the code fallback (SUGGESTED_SUPPORT_PERCENT) is also 10, so an environment where the PARENT migration 20260902010000_donation_checkout_settings was never applied — itself listed here as unprobeable — renders an identical 10% with no stored value at all. A probe cannot separate "updated to 10" from "no row to update". The isolated staging replay and a direct read of platform_settings -> payment -> donationCheckout -> defaultSupportPercent are the authoritative checks'],
  ['20260903000000_first_party_subject_covers', 'the migration replaces known catalog and placeholder cover URLs with first-party subject URLs, while every public image reader already performs the same replacement at render time; rendered pages are intentionally identical before and after application. The staging migration matrix and a direct post-migration database inventory are the authoritative checks'],
  ['20260902010000_donation_checkout_settings', 'the migration seeds nested checkout pricing in platform_settings, but every public reader deliberately falls back to the same validated defaults when the row or nested key is absent; anonymous output is therefore identical before and after application. The authenticated Super Admin save/read matrix and isolated staging replay are the authoritative checks'],
  ['20260831010000_user_nav_preferences', 'the table is RLS-scoped to `user_id = auth.uid()`, so no anonymous request can observe it at all — a public probe sees the same 401/empty result whether or not the migration is applied. The reader (lib/nav-customization-server.ts) degrades to "no customization" on 42P01, so a signed-in page renders the persona-default sidebar either way; only the migration ledger can distinguish the two states'],
  ...
  ['20260901000000_campaign_payout_ready', 'the observable difference is entirely DATA-dependent: after applying, discovery listings drop campaigns whose beneficiary/organizer has no verified connected account. Measured on production 2026-08-09, that is ~15% of live campaigns (4 of 26 sampled), but WHICH campaigns changes continuously as organizers finish Stripe onboarding. A probe naming a specific slug would flip to "not applied" the moment that organizer completed setup, and a probe on the total count has no fixed expected value. The application also fails open by design — lib/campaign-visibility.ts only applies the filter when a column probe definitively finds `payout_ready` — so a public read succeeds identically before and after'],
  ['20260902000000_unified_campaign_builder', 'the unified draft, version, source, settings, and atomic publish model is authenticated and service-role-only; proving it requires the isolated staging builder matrix, while public campaign reads remain compatible before and after application'],
