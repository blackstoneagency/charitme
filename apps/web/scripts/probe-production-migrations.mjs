...
const NO_PUBLIC_SIGNAL = [
  ['20260829000000_platform_impact_stats', 'both tables are gated on published=true and the seed ships every row UNPUBLISHED, so applied-but-unpublished is indistinguishable from not-applied — /impact renders its measured figures either way. That is the intended state: publishing an impact or spend claim is a deliberate act by someone who can source it, not a consequence of running a migration'],
